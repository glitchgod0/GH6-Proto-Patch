...
script edit_cas_layer 
	...
	if NOT structurecontains structure = <mask> no_scale
		if structurecontains structure = <mask> uniform_scale
			...
			<aspect_ratio> = ((1.0 * <x_scale>) / (1.0 * <y_scale>))
			button_widget_add_item {
				widget_id = scale_widget
				text = qs(0x7287553b)
				button_icon = widget_uni_scale
				start_value = <y_scale>
				min = ($min_scale)
				max = ($max_scale)
				step = ($scale_increment)
				pad_left_script = cap_layer_set_uniform_scale
				pad_left_params = {part = <part> cap_index = <cap_index> layer_index = <layer_index> layer = <layer> aspect_ratio = <aspect_ratio>}
				pad_right_script = cap_layer_set_uniform_scale
				pad_right_params = {part = <part> cap_index = <cap_index> layer_index = <layer_index> layer = <layer> aspect_ratio = <aspect_ratio>}
...
script cap_layer_set_uniform_scale 
	x_scale = (<aspect_ratio> * <value>)
	y_scale = <value>
...
script updatecasmodelcap 
	...
	updatecurrentcasmodel buildscript = modelbuilder_update_single_cap_texture flush_assets = <flush_assets> buildscriptparams = {part = <part>}
endscript

script is_matching_section 
	if ((<cap>.base_tex) = (<section>.base_tex))
		return \{true}
	endif
	return \{false}
endscript
	...
	elseif NOT gotparam \{div_id}
		...
		if (<array_size> > 0)
			...
			if ((<divisions> [<i>].desc_id) = <div_id>)
				if ((<target>.base_tex) = (<divisions> [<i>].base_tex))
					return \{true}
				endif
			endif
			i = (<i> + 1)
...
script get_div_info 
	...
	if gotparam \{divisions}
		...
		if (<array_size> > 0)
			...
			begin
			if ((<divisions> [<i>].desc_id) = <div_id>)
				return true base_tex = (<divisions> [<i>].base_tex)
			endif
			i = (<i> + 1)
			repeat <array_size>
		endif
...
script add_default_cap_array_to_newstruct \{part = !q1768515945
