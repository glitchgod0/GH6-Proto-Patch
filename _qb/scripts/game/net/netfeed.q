	...
	switch <event>
		...
		if netfeed func = getmsgheader msgid = <msgid> userid = <userid> out = msg_header
			if friend_feed_get_message_string_from_message {
					...
					msg_type = `default`
					msg = <msg>
					gamertag_string = (<msg_header>.sendername)
				...
				0xb540620a feed_data = <feed_data>
			else
				netfeed \{func = logmessage
					msg = 'netfeedqeventhandler: Failed getting message string. Message dropped.'}
			endif
		else
			netfeed \{func = logmessage
				msg = 'netfeedqeventhandler: Failed to get header. Message dropped.'}
		endif
