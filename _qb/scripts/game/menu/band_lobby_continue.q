...
script band_lobby_continue_check 
	...
	if (<check_type> != skip)
		...
		<mic_controller> = <controller>
		if NOT band_lobby_continue_check_mic_distribution
			band_lobby_change_focus_submenu_all \{focus_type = unfocus}
			ui_event_wait event = menu_change data = {is_popup state = uistate_band_lobby_popup controller = <mic_controller> reason = <reason> message = <message>}
			return \{false
				popup = 1}
		endif
		getnumplayersingame
...
script band_lobby_continue_career 
	...
	if NOT (gman_goalisactive goal = career)
		...
		gman_quickstartgoal \{goal_struct = career_info
			goal_name = career}
		begin
		...
		if NOT has_completed_forced_career_flow device_num = <controller>
			band_lobby_continue_menu_change new_state = uistate_career_movie data = {movie = 'career_intro' new_state = uistate_quest_map device_num = <controller> new_data = {device_num = <controller>}}
		else
...
script band_lobby_continue_menu_change \{data = {
		}}
	requireparams \{[
			new_state
		]
