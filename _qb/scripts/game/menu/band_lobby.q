		...
		if ((<connections> [<i>].is_local) = 0)
			if band_lobby_is_new_party_member user_id = (<connections> [<i>].user_id) party_id = (<connections> [<i>].party_id)
				...
				if NOT (<submenu_data_index> = -1)
					if bl_is_managing
						band_lobby_manage_band_members action = deselect player = (<managed_submenu_index> + 1) submenu_index = <managed_submenu_index>
						band_lobby_wait_for_menu_change
					endif
...
script band_lobby_leave_party 
	...
	if (<occupied> = 1)
		if playerinfoequals <player> is_local_client = 1
			...
			band_lobby_change_menu action = back menu = main player = <player> submenu_index = <i> no_forced focus = 1
		else
...
script band_lobby_get_submenu_data 
	requireparams \{[
		...
		all}
	printf channel = band_lobby qs(0x46e4020d) c = <submenu_index>
	data = {}
	formattext checksumname = submenu_data 'band_lobby_submenu_data_%d' d = <submenu_index>
	...
	if ((<controller_to_player_array> [<i>]) >= 0)
		...
		if checkforsignin controller_index = <i> dont_set_primary
			...
			if isps3
				...
				band_lobby_get_submenu_data submenu_index = <i> param = gamertag_checksum
				change globalname = <gamertag_checksum> newvalue = ($<gamertag>)
			endif
		endif
...
script bl_toggle_vocal_star_power_tap 
	requireparams \{[
			player
		]
		all}
...
script bl_toggle_vocals_mic_enhancement \{player = !i1768515945}
	options_change_vocals_mic_enhancement <...>
	obj_getid
	band_lobby_refresh_menu_text id = <objid>
endscript
