...
script setup_highway \{player = 1
	...
	hdims = ((<pos_table>.highway_top_width) * (1.0, 0.0))
	create_highway_material player = <player> texture = `tex\models\highway\highway_guitar_01.dds`
...
script destroy_highway 
	...
	destroyscreenelement id = <name>
	spawnscriptnow \{audio_crowd_kill_sounds_at_highway_destroy}
	getarraysize \{$gem_colors}
	array_count = 0
	begin
	color = ($gem_colors [<array_count>])
...
script highway_visible 
	change \{g_debug_highway_visible = 1}
endscript

script highway_invisible 
	change \{g_debug_highway_visible = 0}
endscript
...
script update_highway_scale \{player = 1
	...
	if NOT faststructurecontains structure = <pos_table> name = widthoffsetfactor
		return
	endif
	...
	gem_container = <extended_checksum>
	if screenelementexistssimpleid id = <gem_container>
		fastgetscreenelementposition id = <gem_container>
		x = (<screen_element_pos>.(1.0, 0.0))
		y = (<highway_height> - (<highway_height> * <y_scale_factor>))
		p = (((1.0, 0.0) * <x>) + ((0.0, 1.0) * <y>))
		fastsetscreenelementprops id = <gem_container> new_props = {pos = <p>}
	endif
	...
	if (<morphtarget> = 1)
		...
		rescalepostableusingmorphtargets {
			...
			start_y = <starty>
			height = <height>
			scale_y = <y_scale_factor>
			pos_index = <pos_index>
			extra_width_offset_factor = <extra_offset_factor>
			extra_bottom_x_scale = <extra_bottom_x_scale>
			height_offset_factor = <heightoffsetfactor>
			height_offset_exp = <heightoffsetexp>
	...
	highway_sprite = <extended_checksum>
	fastsetscreenelementprops id = <highway_sprite> new_props = {scale = <hscale> pos = <hpos>}
	extendcrcwithinteger checksum = fretbar_containerp integer = <player>
	fretbar_id = <extended_checksum>
	fastsetscreenelementprops id = <fretbar_id> new_props = {scale = <hscale>}
	height = 0.0
	height = (<pos_table>.highway_height)
	top_width = 0.0
	...
	sidebar_left_container = <extended_checksum>
	if screenelementexistssimpleid id = <sidebar_left_container>
		fastsetscreenelementprops id = <sidebar_left_container> new_props = {rot_angle = <angle> pos = <scaled_lpos>}
	endif
	...
	sidebar_right_container = <extended_checksum>
	if screenelementexistssimpleid id = <sidebar_right_container>
		fastsetscreenelementprops id = <sidebar_right_container> new_props = {pos = <scaled_rpos> rot_angle = (0.0 - <angle>)}
	endif
	...
	if (<jamroom_highway> = 0)
		...
		nowbar_id = <extended_checksum>
		if screenelementexistssimpleid id = <nowbar_id>
			fastsetscreenelementprops id = <nowbar_id> new_props = {scale = <nowbar_scale> pos = <p>}
		endif
	endif
	y = (<highway_height> - (<highway_height> * <y_scale_factor>))
	offset = ((0.0, 1.0) * (-1 * <y>))
	...
	left_rock_id = <extended_checksum>
	pos = (<rockmeter_pos> + <offset>)
	if screenelementexistssimpleid id = <left_rock_id>
...
script update_whammy_angles \{player = 1
	...
	rows = $highway_lines
	normal_rows = $real_highway_lines
	if NOT playerinfochecksumequals player = <player> checksum = part value = drum
		...
		if fastusefourlanehighway player = <player>
			gts = (<top_width> / 4.0)
		endif
		rsx = (<htx> + (<gts> * 0.5))
		ysx = (<rsx> + <gts>)
		bsx = (<ysx> + <gts>)
		if NOT fastusefourlanehighway player = <player>
			osx = (<bsx> + <gts>)
			gsx = (<osx> + <gts>)
		else
			osx = (<bsx> + (10 * <gts>))
	...
	pa = <atan>
	if playerinfochecksumequals player = <player> checksum = part value = drum
	...
	if ((playerinfochecksumequals player = <player> checksum = part value = guitar) || (playerinfochecksumequals player = <player> checksum = part value = bass))
...
script updatestringangles \{player = 1
		nb_scale_ar = !f1768515945}
	color_string = ''
	string_scale = (0.0, 0.0)
	array_size = 0
	fastgetarraysize \{array = $gem_colors}
	array_count = 0
	begin
	color = ($gem_colors [<array_count>])
	if faststructurecontains structure = (($button_up_models [(<player> -1)]).<color>) name = name
		<color_string> = (($button_up_models [(<player> -1)]).<color>.name_string)
		extended_checksum = ``
		extendcrcwithstring checksum = <extended_checksum> string = <color_string>
		extendcrcwithstring checksum = <extended_checksum> string = '_stringp'
		extendcrcwithinteger checksum = <extended_checksum> integer = <player>
		name_string = <extended_checksum>
		extended_checksum = ``
		extendcrcwithstring checksum = <extended_checksum> string = <color_string>
		extendcrcwithstring checksum = <extended_checksum> string = '_touch_glowp'
		extendcrcwithinteger checksum = <extended_checksum> integer = <player>
		name_touch_glow = <extended_checksum>
		if (<color> != white)
			get_new_string_scale player = <player> string_id = <name_string> nb_scale_diff = <nb_scale_ar>
			fastsetscreenelementprops id = <name_string> new_props = {
				rot_angle = (($button_models [(<player> -1)]).<color>.angle)
				scale = <string_scale>
			}
			props = ($touch_glow_props)
			if screenelementexistssimpleid id = <name_touch_glow>
				fastsetscreenelementprops id = <name_touch_glow> new_props = {
...
script get_new_string_scale \{player = !i1768515945
		string_id = !q1768515945
		nb_scale_diff = !f1768515945}
	pos_index = (<player> - 1)
...
script highway_scale_manager \{jamroom_highway = 0
	...
	if (($g_has_inited_highway_scale_targets) = 0)
		...
		changeglobalinteger \{global_name = g_has_inited_highway_scale_targets
			new_value = 1}
	endif
	start_x_scale = 0.0
	dest_x_scale = 0.0
	...
	begin
	tag_value = 0.0
	if getfloattag object_name = <objid> tag_name = lerping_factor
		lerping_factor = <tag_value>
		if NOT (<lerping_factor> = 1.0)
			if getfloattag object_name = <objid> tag_name = start_x_scale
				start_x_scale = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = dest_x_scale
				dest_x_scale = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = start_y_scale
				start_y_scale = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = dest_y_scale
				dest_y_scale = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = start_extra_offset_factor
				start_extra_offset_factor = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = dest_extra_offset_factor
				dest_extra_offset_factor = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = start_x_pos
				start_x_pos = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = dest_x_pos
				dest_x_pos = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = start_heightoffsetfactor
				start_heightoffsetfactor = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = dest_heightoffsetfactor
				dest_heightoffsetfactor = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = start_heightoffsetexp
				start_heightoffsetexp = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = dest_heightoffsetexp
				dest_heightoffsetexp = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = start_y_pos
				start_y_pos = <tag_value>
			endif
			if getfloattag object_name = <objid> tag_name = dest_y_pos
				dest_y_pos = <tag_value>
			endif
			update_highway_value dest = 1 curr = <lerping_factor> max_delta = ($delta_lerp)
			lerping_factor = <next>
			result = 0.0
			fastlinearmap from = <start_x_scale> to = <dest_x_scale> based_on = <lerping_factor> lower_bound = 0.0 upper_bound = 1.0
			next_x_scale = <result>
			fastlinearmap from = <start_y_scale> to = <dest_y_scale> based_on = <lerping_factor> lower_bound = 0.0 upper_bound = 1.0
			next_y_scale = <result>
			fastlinearmap from = <start_extra_offset_factor> to = <dest_extra_offset_factor> based_on = <lerping_factor> lower_bound = 0.0 upper_bound = 1.0
			next_extra_offset_factor = <result>
			fastlinearmap from = <start_x_pos> to = <dest_x_pos> based_on = <lerping_factor> lower_bound = 0.0 upper_bound = 1.0
			next_x_pos = <result>
			fastlinearmap from = <start_heightoffsetfactor> to = <dest_heightoffsetfactor> based_on = <lerping_factor> lower_bound = 0.0 upper_bound = 1.0
			next_heightoffsetfactor = <result>
			fastlinearmap from = <start_heightoffsetexp> to = <dest_heightoffsetexp> based_on = <lerping_factor> lower_bound = 0.0 upper_bound = 1.0
			next_heightoffsetexp = <result>
			calculate_y_pos_factor_from_lerp_factor lerping_factor = <lerping_factor>
			fastlinearmap from = <start_y_pos> to = <dest_y_pos> based_on = <y_pos_factor> lower_bound = 0.0 upper_bound = 1.0
			next_y_pos = <result>
...
script get_highway_curr_x_scale \{player = 1}
	...
	hw_container_id = <extended_checksum>
	x_scale = 1.0
	y_scale = 1.0
	if screenelementexistssimpleid id = <hw_container_id>
		curr_x_scale = 1.0
		curr_y_scale = 1.0
		<hw_container_id> :gettags
...
script adjust_highway_rails_to_scale \{player = 1}
	...
	name = <extended_checksum>
	if screenelementexistssimpleid id = <name>
...
script getcurrentyposandscale \{player = 1}
	...
	curr_y_scale = 0.0
	extendcrcwithinteger checksum = highway_containerp integer = <player>
	<extended_checksum> :getsingletag curr_y_pos
	<extended_checksum> :getsingletag curr_y_scale
