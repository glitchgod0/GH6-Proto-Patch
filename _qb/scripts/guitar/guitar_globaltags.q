default_globaltag_checksum = 0xd192cec2
debug_saves = 0
...
script reset_globaltags savegame = ($primary_controller) async = 0
	...
	conditional_async_wait async = <async> text = qs(0x8bfbe280)
	...
	setglobaltags savegame = <savegame> user_options params = {
		facebook_info = {username = qs(0x03ac90f0) password = qs(0x03ac90f0) broadcast_enabled = 1}
		twitter_info = {username = qs(0x03ac90f0) password = qs(0x03ac90f0) broadcast_enabled = 1}
	}
...
script setup_progression_slots \{async = 0}
	...
	if demobuild
		<progression_name> = ($g_demo_default_band_name)
	else
		cas_random_band_name
		<progression_name> = <random_name>
	endif
...
script 0x4cb6620f \{part = 'guitar'}
	formattext checksumname = song '%s' s = (<song_prefix>)
	...
	if NOT getglobaltags <song> savegame = <savegame> param = <beststars> noassert = <noassert>
		return \{false}
	endif
	getglobaltags <song> savegame = <savegame> param = <bestscore> noassert = <noassert>
	getglobaltags <song> savegame = <savegame> param = <bestpercentage> noassert = <noassert>
	return {setlist_bestscore = (<...>.<beststars>)
		setlist_beststars = (<...>.<bestscore>)
		setlist_bestpercentage = (<...>.<bestpercentage>)}
endscript
...
script set_globaltag_song_info \{part = 'guitar'}
	...
	setglobaltags <song> savegame = <savegame> param = <params>
endscript
...
script reset_transient_tags 
	...
	if NOT structurecontains structure = ($levelzones.<venue>) debug_only
		...
		setglobaltags savegame = <savegame> venue_intro_flags params = {<zone_struct>}
		setglobaltags savegame = <savegame> venue_encore_flags params = {<zone_struct>}
		setglobaltags savegame = <savegame> celeb_intro_flags params = {<zone_struct>}
	endif
...
script setup_songlist_tags 
	requireparams \{[
		...
		all}
	setglobaltags savegame = <savegame> songlist_options params = {show_more_info = ($g_songlist_show_more_info) show_ghtunes = ($g_songlist_show_ghtunes) 0x1d79954f = ($0x42a06e7f)}
endscript
