...
script scriptevents_iterator 
	...
	if globalexists name = <intro_script_events> type = array
		...
		<array_entry> = 0
		begin
		timemarkerreached_setparams time_offset = 0 array = <intro_script_events> array_entry = <array_entry> arrayofstructures
		begin
		if timemarkerreached
			getsongtimems time_offset = <time_offset>
			break
		...
		if scriptexists <scriptname>
			spawnscriptnow <scriptname> params = {time = <time> event_time = (($<intro_script_events> [<array_entry>]).time) ($<intro_script_events> [<array_entry>].params)} id = song_event_scripts
		elseif symboliscfunc <scriptname>
			<scriptname> {time = <time> event_time = (($<intro_script_events> [<array_entry>]).time) ($<intro_script_events> [<array_entry>].params)}
		endif
		<array_entry> = (<array_entry> + 1)
		repeat <array_size>
	else
