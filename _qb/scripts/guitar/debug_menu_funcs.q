...
script debug_menu_num_players_choose 
	...
	gamemode_gettype
	if (<type> = career)
		setglobaltags savegame = ($primary_controller) career_progression_tags params = {current_chapter = quest_chapter_axel} progression = true
	endif
endscript
...
script debug_menu_play_celeb_intro_get_value 
	return value = ($debug_selected_intro)
endscript

script debug_menu_play_celeb_intro_get_text 
	requireparams \{[
			value
		]
		all}
	challenge_checksum = ($gig_celeb_intros [<value>])
	venue_checksum = ($<challenge_checksum>.venue)
...
script debug_menu_play_celeb_intro_option_left 
	requireparams \{[
			value
		]
		all}
	...
	if (<new_value> < 0)
		...
		<new_value> = (<array_size> - 1)
	endif
	change debug_selected_intro = <new_value>
endscript

script debug_menu_play_celeb_intro_option_right 
	requireparams \{[
			value
		]
		all}
...
script debug_start_song_with_celeb_intro 
	...
	challenge_checksum = ($gig_celeb_intros [$debug_selected_intro])
	song_name = ($<challenge_checksum>.predefined_playlist [0])
	venue = ($<challenge_checksum>.venue)
	...
	debug_select_venue level = <venue> norestart = 1
endscript
...
script 0x517a6414 
	...
	<i> = 0
	begin
	<desc_id> = ((<outfit_array> [<i>]).desc_id)
	if cas_item_is_visible part = cas_full_body desc_id = <desc_id>
		if ghrocker_body_matches matcheswith = (<profile_struct>.name) part = cas_full_body desc_id = <desc_id>
			if is_part_unlocked part = cas_full_body desc_id = <desc_id> savegame = <savegame>
				addarrayelement array = <array> element = {frontend_desc = ((<outfit_array> [<i>]).frontend_desc) outfit_name = ((<outfit_array> [<i>].desc_id)) profile_struct = <profile_struct> part = <part> savegame = <savegame>}
			endif
		endif
	endif
	<i> = (<i> + 1)
...
script 0x5be63cba 
	requireparams \{[
		...
		all}
	return pad_choose_script = 0xdcbbd1a9 pad_choose_params = {index = <index> part = <part> 0xe9c6ec54 = (<profile_struct>.name) outfit_name = <outfit_name> savegame = <savegame>}
endscript
...
script 0xdcbbd1a9 
	...
	if debug_is_part_in_band part = <part>
		...
		globaltag_save_band_member_choice savegame = <savegame> index = <stored_character_index> character_id = <0xe9c6ec54> character_savegame = <savegame>
	endif
...
script debug_menu_build_camera_cut_group_menu 
	...
	index = 0
	begin
	formattext checksumname = camera_array '%s_%p' s = <pakname> p = ($cameracutprefixarray [<index>])
	if globalexists name = <camera_array>
		getarraysize $<camera_array>
		if (<array_size> > 0)
			formattext textname = group_name qs(0x581d2af2) p = ($cameracutprefixarray [<index>])
...
script debug_cycle_cameracut \{direction = !q1768515945}
	...
	formattext checksumname = camera_array_1 '%s_%p' s = <pakname> p = ($cameracutprefixarray [($debug_cameracut_group_iterator)])
