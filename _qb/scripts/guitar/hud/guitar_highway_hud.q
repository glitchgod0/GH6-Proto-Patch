...
script hud_update_progress_meter 
	...
	<scale> = (((1.0, 0.0) * <x_scale>) + (0.0, 1.0))
	se_setprops progress_bar_scale = <scale>
	wait \{5
		gameframes}
	repeat
endscript
...
script setup_challenge_meter \{player = 1}
	...
	if gotparam \{challenge_base_name}
		...
		<the_id> :se_setprops challenge_icon_texture = <icon_name>
		<the_id> :se_setprops challenge_icon_alpha = 1.0
	endif
endscript
...
script fill_challenge_meter_to \{player = 1
	...
	if gotparam \{challenge_meter_id}
		...
		<the_id> :gettags current_friend_level
	endif
	...
	if gotparam \{use_friend_target}
		if (<friend_level> > <current_friend_level>)
			friend_challenge_complete_fx id = <the_id> player = <player>
			<the_id> :settags current_friend_level = <friend_level>
			<the_id> :se_setprops gamer_tag_text = <friend_name>
		endif
		if gotparam \{challenge_meter_id}
			<the_id> :se_setprops gamer_tag_text = <friend_name>
		endif
