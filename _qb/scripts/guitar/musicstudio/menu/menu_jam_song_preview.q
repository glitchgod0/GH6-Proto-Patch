...
script remove_missing_album_cover_textures 
	printscriptinfo \{'remove_missing_album_cover_textures'}
	if getcapsectionsarray \{appearance = {
		...
		if structurecontains structure = (<album_cover> [<i>]) name = layers
			...
			if (<remove> = 1)
				...
				if structurecontains structure = (<curr_layers> [<j>]) flags
					if ((<curr_layers> [<j>].flags) >= 256)
						updated_flags = ((<curr_layers> [<j>].flags) - 256)
