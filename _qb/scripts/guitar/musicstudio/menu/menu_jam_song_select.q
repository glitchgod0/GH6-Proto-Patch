...
script create_jam_song_select_popup \{advanced_record = 0}
	...
	pushdisablerendering \{context = create_jam_song_select_popup}
	...
	popdisablerendering \{context = create_jam_song_select_popup}
...
script musicstudio_initialize_song \{skip_instrument_load = 0}
	getsonginfo
...
script jam_auto_generate_vocals_gem_array 
	...
	if (<track_size> > 0)
		begin
		0x477e9e04 track = ($jam_tracks [<instrument>].id) index = <counter>
		if (<vocal_skip> = 1)
			...
			if (<end_phrase> = 1)
				...
				if (<vocal_join> = 1 &&
					...
					if (<counter> + 1 < <track_size>)
						0x477e9e04 track = ($jam_tracks [<instrument>].id) index = (<counter> + 1)
						if (<vocal_skip> = 0 &&
								<vocal_spoken> = 0)
	...
	getarraysize ($jamsession_song_vocals)
	change 0x26d119cd = <array_size>
	getarraysize ($jamsession_lyrics)
	change 0xb0281b7d = <array_size>
	getarraysize ($jamsession_vocals_markers)
	change 0xd622fa2a = <array_size>
	getarraysize ($jamsession_vocals_phrases)
	change 0xb25b38cf = <array_size>
endscript
...
script start_jam_song \{difficulty = hard}
	...
	sfx_backgrounds_new_area \{bg_sfx_area = frontend_menu_music}
