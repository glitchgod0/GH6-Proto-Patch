...
script musicstudio_update_fretbar 
	...
	if NOT (<fretbar_num> = 0)
		...
		text_offset = (618.0, -2.0)
		rgba = [92 59 61 255]
		musicstudio_count_get_note_intervals note_value = 4 time = <time>
		...
		if musicstudio_ghmix :desc_resolvealias \{name = alias_ghmix_highway
			...
			if <alias_ghmix_highway> :desc_resolvealias name = ghmix_nowbar param = ghmix_nowbar
				...
				switch <jam_instrument>
					...
					casttointeger \{curr_velocity}
					if (<curr_velocity> > 0)
						formattext textname = open_velocity_text qs(0xb3fe6741) s = <curr_velocity>
					else
						<open_velocity_text> = qs(0x0796ce39)
					endif
...
script musicstudio_stop_scrub_note 
	musicstudio_mainobj :musicstudioghmix_getnotequantizems note_value = (1.0 / 2.0)
...
script musicstudio_update_count 
	...
	if screenelementexists \{id = musicstudio_ghmix}
		if musicstudio_ghmix :desc_resolvealias \{name = alias_ghmix_highway
				param = alias_ghmix_highway}
			if <alias_ghmix_highway> :desc_resolvealias name = ghmix_nowbar param = ghmix_nowbar
				musicstudio_mainobj :musicstudioghmix_getnoteintervals note_value = 1.0 time = <time>
				...
				musicstudio_mainobj :musicstudioghmix_getnoteintervals note_value = (1.0 / 4.0) time = <time>
				mod a = <intervals> b = 4
				formattext textname = beat_text qs(0x73307931) s = (<mod> + 1)
				<ghmix_nowbar> :setprops beat_text = <beat_text>
				musicstudio_count_get_note_intervals note_value = 48 per_note = 0.25 time = <time>
				if (<intervals> <= 9)
...
script musicstudio_count_get_note_intervals \{per_note = 1.0}
	musicstudio_mainobj :musicstudioghmix_getquantizetime time = <time> snap = (<per_note> / <note_value>)
	musicstudio_mainobj :musicstudioghmix_getnotequantizems note_value = (<per_note> / <note_value>)
