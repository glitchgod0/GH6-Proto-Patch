...
0x3b6cccf7 = -1
0x9260476c = 0
notetracker_quickedit_dropnote_snap_forward_ms = 80
notetracker_quickedit_dropnote_snap_backward_ms = 40
...
script musicstudio_notetracker_init \{editing = 0}
	...
	printf qs(0xbc0fd4f8) s = <filename>
...
script notetracker_quickedit_select_note 
	...
	musicstudio_mainobj :musicstudioghmix_findgem <findgem_params> button = all include_sustain = false
	<all_index> = <index>
	...
	if (<index> = -1 &&
		...
		if (<index> = -1)
			...
			action_params = {action = 0xf2666525}
		endif
	endif
	if NOT gotparam \{0x5057f086}
		if ($notetracker_note_held = 1)
			return
		endif
		change \{notetracker_note_held = 1}
		change notetracker_note_held_time = <song_time>
		change 0x932727dc = <index>
	...
	if gotparam \{action}
		...
		musicstudio_mainobj :musicstudioghmix_performgemaction <action_params>
...
script notetracker_quickedit_select_note_vocals 
	...
	elseif ($notetracker_quickedit_mode = pitch)
		switch <button>
			...
			action_params = {action = singpitch}
			auto_repeat = 1
			wait_time = 0
			0x24b110da = 0
			case yellow
			...
			if (<index> != -1)
				musicstudio_mainobj :musicstudio_getcurrmainplayer
				<main_player> :musicstudioplayer_getinstrument
				0x477e9e04 track = ($jam_tracks [<instrument>].id) index = <index>
...
script notetracker_quickedit_autorepeat \{wait_time = 400
		0x24b110da = 100}
...
script notetracker_quickedit_removeorholdnote \{action = 0x08ad633e}
	...
	wait \{1
		gameframe}
	repeat
	printf qs(0xf5b68ef0) i = <held_down> a = <action>
	if (<held_down> = 0)
		action_params = {
			track = ($jam_tracks [<instrument>].id)
			action = <action>
...
script 0xa4e3ebf2 \{action = pitchdown}
	musicstudio_mainobj :musicstudio_getcurrmainplayer
	<main_player> :musicstudioplayer_getinstrument
	musicstudio_mainobj :musicstudioghmix_getsongtime
	params = {
	...
	if NOT gotparam \{index}
		musicstudio_mainobj :musicstudioghmix_findgem <params> include_sustain = true
	endif
	musicstudio_mainobj :musicstudioghmix_performgemaction <params> index = <index>
endscript
...
script notetracker_quickedit_playpitch_spawned \{note_velocity = 100
	...
	if (<vocal_join> = 1)
		if (<index> < <track_size>)
			next_index = (<index> + 1)
			0x477e9e04 track = ($jam_tracks [<instrument>].id) index = <next_index>
			if (<vocal_spoken> = 0)
				if (<vocal_skip> = 0)
					if (<vocal_octave> > 3)
						vocal_octave = 3
...
script notetracker_quickedit_taildrag 
	...
	begin
	musicstudio_mainobj :musicstudioghmix_getsongtime
	if NOT musicstudio_mainobj :musicstudioghmix_updatenotesustain track = ($jam_tracks [(<instrument>)].id) time = <song_time> index = <index>
		break
	endif
...
script notetracker_quickedit_vocal_pitch_indicator_update 
	...
	if (<instrument> != 5)
		...
		if (<pitch> >= 0 &&
				...
				<octave> < 4 &&
				<octave> >= 0)
			change 0x47d88093 = <pitch>
			change 0x3b6cccf7 = <octave>
