...
script drummer_head_look \{num_drummers = !i1768515945
		drummer_ids = !a1768515945
		drummer_offset = !f1768515945}
	lookat_active = 0
	drummer_target_lookat_fastest = ($g_drummer_target_lookat_fastest)
	drummer_target_lookat_slowest = ($g_drummer_target_lookat_slowest)
	drummer_old_target_1 = 0
	drummer_old_target_2 = 0
	begin
	...
	if (<size> > 0)
		...
		target_distance = ((<structure>.time) - <time>)
		drummer_lookat_target = (<structure>.pieces_value_1)
		drummer_old_target_2 = <drummer_old_target_1>
		drummer_old_target_1 = <drummer_lookat_target>
		if (<target_distance> > <drummer_target_lookat_fastest>)
			get_new_note = 0
		else
			get_new_note = 1
		endif
	...
	if (<get_new_note> = 0)
		if (<target_distance> < <drummer_target_lookat_slowest>)
			blend_duration = (<target_distance> / 1000.0)
			blend_duration_out = 0.7
			switch <drummer_lookat_target>
				case 1
				target_offset = (0.32626003, 0.70264995, 0.36839)
				case 2
				target_offset = (0.7356799, 0.94216, 0.46612)
				case 3
				target_offset = (0.55569, 1.267, 0.84543)
				case 4
				target_offset = (0.2066, 0.81957, 0.63966)
				case 5
				target_offset = (-0.1415, 0.8038201, 0.59746003)
				case 6
				target_offset = (-0.24644001, 1.25787, 0.88224995)
				case 7
				target_offset = (-0.21303001, 0.67427, 0.36932)
				case 8
				target_offset = (-0.53343, 1.0296999, 0.53204995)
				case 9
				target_offset = (0.0, 5.0, 0.0)
				default
				target_offset = (0.0, 0.5, 5.0)
			endswitch
			if ($g_drummer_lookat_control_priority_hijacked = 0)
				fastspawnscriptnow script_name = hold_drummer_lookat_on_request params = {
			...
			get_new_note = 1
			waitseconds num_seconds = (<blend_duration> + (($g_drummer_target_lookat_before_hitting_target) / 1000.0))
			lookat_duration = ($g_drummer_target_lookat_before_hitting_target)
			lookat_active = 1
		else
			target_distance = (<target_distance> - <frame_length>)
		endif
	else
		getstructurequeuesize \{queue_name = drum_lookat_queue}
		if (<size> > 0)
			...
			target_distance = ((<structure>.time) - <time>)
			drummer_lookat_target = (<structure>.pieces_value_1)
			if ((<drummer_lookat_target> != <drummer_old_target_1>) && (<drummer_lookat_target> != <drummer_old_target_2>))
				if (<target_distance> > <drummer_target_lookat_fastest>)
					drummer_old_target_2 = <drummer_old_target_1>
					drummer_old_target_1 = <drummer_lookat_target>
					get_new_note = 0
	...
	if (<lookat_active> = 1)
		if (<lookat_duration> > 0.0)
			lookat_duration = (<lookat_duration> - <frame_length>)
		else
			fastspawnscriptnow script_name = hold_drummer_lookat_off_request params = {
				drummer_ids = <drummer_ids>
				blend_duration = <blend_duration_out>
				num_drummers = <num_drummers>
			}
			lookat_active = 0
		endif
...
script drummer_controlscript 
	...
	if NOT fastglobalexists name = <event_array> type = array
		return
	endif
	...
	index = 0
	begin
	(<drummer_ids> [<index>]) :anim_command {
		target = face
		command = degenerateblend_addbranch
		params = {
			tree = $drummer_face_branch_blend
			blendduration = 0.0
			params = {
				idle_anim = drumdroid_facial_loop_idle
				rocking_anim = drumdroid_facial_loop_hard_rocking
			}
		}
	}
	index = (<index> + 1)
	repeat <num_drummers>
	changeglobalinteger \{global_name = g_autodrummer_active
		new_value = 1}
	begin
	getsongtimems time_offset = <drummer_offset>
	if (<time> >= -2000.0)
