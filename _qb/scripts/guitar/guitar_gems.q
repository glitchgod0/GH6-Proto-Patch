...
script get_marker_array_name 
	requireparams \{[
		...
		all}
	get_notetrack_part_text player = <player> song_name = <song_name> part = <part> fullname = 1
	get_song_prefix song = <song_name>
...
script gem_scroller \{player = 1}
	...
	if NOT musicstudio_mainobj :musicstudio_isinmusicstudio
		if ($game_mode != tutorial && $game_mode != practice && $calibrate_lag_enabled = 0 && $notetracker_quickplay_active = 0)
			getfirstplayer \{on_screen
				out = first}
			if (<player> = <first>)
				...
				if ($autovocaltest = 1)
					...
					spawnscriptnow \{autovocal_controlscript}
				endif
	...
	formattext checksumname = input_array 'input_arrayp%p' p = <player>
	if (($calibrate_lag_enabled = 0) || ($calibrate_highway_state = settingstest))
	...
	if (($calibrate_lag_enabled = 1) && ($calibrate_highway_state != settingstest))
		return
	endif
...
script start_gem_scroller \{starttime = 0
	...
	if ($g_memory_check_fragmentation = 1)
		gamecheckfragmentation \{allowed_list = $g_heaps_allowed_fragmentation}
	endif
	...
	spawnscriptnow dynamic_ad_start params = {mode = venue zone = (<zone>)}
	if ($end_credits = 2)
...
script kill_gem_scroller \{no_render = 0
	...
	if NOT (<type> = restart)
		...
		if (<stop_song> = 1)
			songaudiodeinit \{no_script_wait = 1}
		endif
...
script load_venue \{block_scripts = 0}
	...
	spawnscriptnow \{audio_crowd_game_start_cheer}
