...
script team_select_update_player_ui 
	...
	if (<band_request> > 0 && <position> = normal)
		...
		audio_ui_play_switch_teams_sfx type = request direction = <slide_dir>
		se_waitprops
		<waited> = 1
	elseif (<position> = out && <band_request> = 0)
		...
		audio_ui_play_switch_teams_sfx type = resend direction = <slide_dir>
		se_waitprops
		<waited> = 1
	endif
	if (<waited> = 0)
...
script team_select_swap_loop 
	...
	if teamswaploop team_0 = <team_0> team_1 = <team_1>
		...
		wait \{0.2
			seconds}
		popup_teams_ui_update team_0 = ($g_team_0) team_1 = ($g_team_1)
		audio_ui_play_switch_teams_sfx \{type = swap}
		if ininternetmode
...
script team_select_reorder_teams 
	...
	if (<max_players> > 0)
		...
		if (<num_players> > 0)
			...
			if (<num_players_shown> < 4 && <solo_highway_mode> = 0)
				getnumplayersingame
				if (<num_players> = 4)
					getfirstplayer
					begin
