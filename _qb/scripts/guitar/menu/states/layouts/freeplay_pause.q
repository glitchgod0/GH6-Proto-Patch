...
script create_freeplay_menu \{menu_state = !q1768515945
	...
	<resolved_id> :desc_resolvealias name = alias_freeplay_highway_menu_scrollbar param = scrollbar
	<scrollbar> :se_setprops {alpha = <scrolling>}
	if (<scrolling> = 1)
		<menu_id> :se_setprops {
			event_handlers = [
				{menu_selection_changed audio_ui_freeplay_play_sfx params = {type = up player = <player>}}
				{menu_selection_changed 0xefd4ebba params = {menu_id = <menu_id> scrollbar_id = <scrollbar> type = up player = <player> num_entries = <array_size>}}
			]
		}
		0xefd4ebba menu_id = <menu_id> scrollbar_id = <scrollbar> type = up player = <player> num_entries = <array_size> selected_index = <menu_focus_idx>
	endif
	freeplay_set_menu_background_dims menu_parent = <menu_parent> menu_width = <menu_width>
	freeplay_set_menu_highlight_dims menu_id = <menu_id> menu_width = <menu_width> num_elements = <array_size>
...
script 0xefd4ebba 
	...
	<placement> = (80.0 * ((<selected_index> * 1.0) / (<num_entries> - 1)))
	pos = ((0.0, 1.0) * <placement>)
	<scrollbar_id> :se_setprops {
		thumb_pos = <pos>
	}
endscript
...
script freeplay_pause_spawned \{player = !i1768515945
	...
	getplayerinfo <player> controller
	ui_event_wait event = menu_change data = {state = 0x0604ee76 0x3002594a = {mode = <songlist_mode> exclusive_device = <controller>}}
endscript
...
script freeplay_create_stats_ui 
	...
	if (<num_active_players> > 0)
		...
		get_percent_notes_hit player_index = <player> use_player_info = 1
...
script freeplay_pause_generate_entries_venue 
	...
	if NOT structurecontains \{structure = zone_data
			debug_only}
