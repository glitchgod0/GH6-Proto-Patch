	...
	if gotparam \{pad_square_script}
		setscreenelementprops {
			...
			event_handlers = [
				...
				{pad_square ui_sfx params = {menustate = car uitype = select}}
			]
		}
	endif
	if gotparam \{pad_start_script}
		setscreenelementprops {
			...
			event_handlers = [
				...
				{pad_start ui_sfx params = {menustate = car uitype = select}}
			]
		}
	endif
endscript
...
script save_instrument_accept_name 
	fire_achievement name = achievement_customizer controller = ($primary_controller)
