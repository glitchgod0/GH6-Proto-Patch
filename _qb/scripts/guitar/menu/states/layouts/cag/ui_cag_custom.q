...
script ui_create_cag_custom 
	...
	if ((<instrument_info>.desc_id) = guitar)
		<title> = qs(0x9504b94a)
		<pad_back_script> = ui_cag_custom_back
	elseif ((<instrument_info>.desc_id) = bass)
		<title> = qs(0x7d4f9214)
		<pad_back_script> = ui_cab_custom_back
	endif
	...
	if get_section_index_from_desc_id part = (<instrument_info>.neck_part) target_desc_id = finishes
		add_car_menu_text_item {
			icon = icon_cag_inlay
			text = qs(0x3436c336)
...
script ui_cag_custom_back 
	ui_sfx \{menustate = car
		uitype = back}
	if structurecontains structure = ($cas_current_appearance) cag_preset_index
		0x8c9fecc8 index = ($cas_current_appearance.cag_preset_index)
	endif
	generic_event_back_block
endscript

script ui_cab_custom_back 
	ui_sfx \{menustate = car
		uitype = back}
	if structurecontains structure = ($cas_current_appearance) 0xfc00902f
		0xc47fe2ac index = ($cas_current_appearance.0xfc00902f)
	endif
	generic_event_back_block
endscript
