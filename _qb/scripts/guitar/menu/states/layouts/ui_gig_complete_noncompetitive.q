...
script ui_create_gig_complete_noncompetitive 
	...
	if (<type> = career)
		...
		if (<current_challenge> != none)
			...
			gman_quickremovegoal goal_name = <current_challenge>
		endif
	else
	...
	if screenelementexists \{id = gig_complete_noncompetitive_id}
		...
		if (<playlist_size> > 0)
			...
			if gig_complete_noncompetitive_id :desc_resolvealias name = <songs_menu_alias>
				...
				total_number_rgba = black
				if (<num_bonus_stars_earned> > 0)
					total_number_rgba = gh5_gold_md
				endif
				gig_complete_noncompetitive_id :se_setprops {
...
script ui_gig_complete_noncompetitive_continue_to_next_screen \{device_num = !i1768515945}
...
script ui_gig_complete_handle_challenge_strip 
	if screenelementexists \{id = gig_complete_noncompetitive_id}
		...
		if (<type> = career && $end_credits = 0)
			...
			<max_challenge_stars> = (<max_challenge_stars> + ($star_reward_gold))
			<max_challenge_stars> = (<max_challenge_stars> + ($star_reward_platinum))
			<max_challenge_stars> = (<max_challenge_stars> + ($star_reward_diamond))
