...
text_characters_and_limited_symbols = [
	[
		qs(0x713755f7)
		qs(0x713755f7)
	]
...
script text_entry_add_item \{pad_back_script = generic_event_back
	...
	change curr_text_characters = ($default_text_characters)
	if gotparam \{numbers_only}
		change curr_text_characters = ($number_text_characters)
	endif
	if gotparam \{characters_and_limited_symbols}
		change curr_text_characters = ($text_characters_and_limited_symbols)
	endif
	createscreenelement {
	...
	setscreenelementprops {
		id = <text_entry>
		event_handlers = [
			{focus <focus_script> params = {text_entry = <text_entry> rgba = <rgba> <focus_params>}}
			{unfocus <unfocus_script> params = {text_entry = <text_entry> rgba = <rgba> <unfocus_params>}}
...
script change_char_up 
	<text_entry> :gettags
	getarraysize ($curr_text_characters)
...
script refresh_text_on_screen 
	...
	new_string = (<new_string> + (($curr_text_characters) [<text_index>] [<caps_lock_state>]))
...
script confirm_name 
	...
	if NOT is_text_valid text_entry = <text_entry>
		...
		generic_event_choose no_sound state = uistate_generic_dialog_box data = {template = continue confirm_func = return_reinit_text_entry confirm_func_params = {text_entry = <text_entry>} is_popup heading_text = qs(0x2a1c09b9) body_text = <validity_message>}
...
script get_string_from_array 
	...
	cap_state = (<array> [<i>] [1])
	text = (<text> + (($curr_text_characters) [<text_index>] [<cap_state>]))
	i = (<i> + 1)
	repeat <index>
	return text = <text>
endscript
...
script initiate_text_entry 
	...
	if (<linear_flow> = true)
		event_handlers = [
			{pad_up change_char_up params = {text_entry = <text_entry>}}
			{pad_down change_char_down params = {text_entry = <text_entry>}}
	...
	if checksumequals a = <text_case> b = mixed
		addarrayelement array = <event_handlers> element = {{pad_option2 toggle_caps_lock params = {text_entry = <text_entry>}}}
...
script is_text_valid 
	...
	if musicstudio_mainobj :musicstudio_isinmusicstudio
		if ui_event_exists_in_stack \{name = 'jam_publish_text_entry'}
			if NOT musicstudio_is_text_valid <...>
				return \{false}
			endif
		endif
	endif
...
script seed_text_entry 
	...
	if (<string_size> > 0)
		...
		j = 0
		begin
		if (<char_array> [<i>] = ($curr_text_characters) [<j>] [0])
			new_val = [0 0]
		...
		if (<char_array> [<i>] = ($curr_text_characters) [<j>] [1])
			new_val = [0 1]
			setarrayelement arrayname = new_val index = 0 newvalue = <j>
			setarrayelement arrayname = text_array index = <i> newvalue = <new_val>
			caps_lock_state = (<text_array> [<i>] [1])
...
script toggle_caps_lock 
	...
	get_string_from_array array = <char_index_array> index = (<current_index> + 1)
	setscreenelementprops id = <text_entry_text> text = <text>
	if NOT ((($curr_text_characters) [(<character> [0])] [0]) = (($curr_text_characters) [(<character> [0])] [1]))
...
script focus_text_entry 
	...
	setscreenelementprops id = <text_entry_marker> rgba = (($default_color_scheme).title_color)
endscript
