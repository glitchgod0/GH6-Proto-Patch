		...
		if structurecontains structure = $download_songlist_props_musicstore <song_name>
			...
			if (<should_include> = 1)
				...
				if NOT music_store_should_hide_item struct = <songprops>
					...
					printf '%s has been marked as hide on platform.' s = (<songprops>.title)
				endif
			endif
		else
...
script issongselectable 
	...
	if NOT songlist_component :getsingletag \{band_instrument_setup}
		return \{true}
	endif
...
script get_ui_song_struct_items 
	...
	if NOT song_is_jamsession song = <song>
		...
		getglobaltags <song> savegame = <savegame> param = num_times_played
	endif
	<num_times_played> = (<num_times_played> * -1)
	...
	if structurecontains structure = <song_struct> playback_track1
		...
		if structurecontains structure = <song_struct> additional_props
